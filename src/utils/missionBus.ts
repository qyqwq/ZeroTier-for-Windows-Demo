...
let createPolling = (netId: string) => {
  const netWorkDetail = () => window.nodeAPI.requestApi({
    url: 'network/' + netId,
    method: 'get'
  }).then(res => {
...
// ==============================================================================
//合并adminId 
const assignAdminId = (netId: string, userId?: string, type?: 'add' | 'red') => {
        ...
        if (res.status == 'success') {
          if (Array.isArray(res.data)) {
            ...
            if (isArr.includes(netId)) {
              let adminIds = assignAdminId(netId, String(zerotierStatus.address))
              //更新权限token到本地文件
              updateLoaclNetwork({
                id: netId,
                Authorization: 'token ' + token,
...
const syncNetworkMember = (netId: string) => {
  ...
  const updateMember = () => window.nodeAPI.requestApi({
    type: 'official',
    ...
    let memberList = res.data.map((mem: any) => {
      let {
        networkId, nodeId, lastSeen, config, name
      } = mem
      let {
        ipAssignments, authorized, id
      } = config
      return {
        id, authorized, lastSeen, nodeId, networkId, name,
        ip: ipAssignments.join('')
      }
    })
    let memberIps = memberList.map((me: any) => {
...
//检查成员昵称
const checkMemberName = (netId: string, member: netMember) => new Promise((resolve, reject) => {
  let net = getNetworkById(netId)
  let memberLocal = getMemberById(net, String(member.id))
  //昵称不匹配，更新昵称
  if (memberLocal && memberLocal.name != member.name) {
    sendlog.log('昵称不匹配，更新昵称 ', `id:${member.id} name:${memberLocal.name} -> ${member.name}`)
...
//更新成员信息
const updateMemberData = (netId: string, member: netMember) => new Promise((resolve, reject) => {
  let net = getNetworkById(netId)
  window.nodeAPI.requestApi({
    type: 'official',
...
//接收node接口数据
const onNodeServeData = () => new Promise((resolve, reject) => {
